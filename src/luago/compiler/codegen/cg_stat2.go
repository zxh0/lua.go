...
func isLocalFuncDef(node *LocalAssignStat) bool {
	if len(node.ExpList) == 1 {
		exp0 := node.ExpList[0]
		if fd, ok := exp0.(*FuncDefExp); ok && !fd.IsAno {
			return true
		}
	}
	return false
...
func (self *codeGen) cgLocalAssignStat0(node *LocalAssignStat) {
	if len(node.ExpList) == 1 {
		exp0 := node.ExpList[0]
		if fd, ok := exp0.(*FuncDefExp); ok && !fd.IsAno {
...
func (self *codeGen) cgAssignStat0(node *AssignStat) {
